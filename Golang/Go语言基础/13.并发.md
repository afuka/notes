...
当main()函数返回的时候该`goroutine`就结束了，所有在main()函数中启动的`goroutine`会一同结束，main函数所在的`goroutine`就像是权利的游戏中的夜王，其他的`goroutine`都是异鬼，夜王一死它转化的那些异鬼也就全部GG了。
...
像这种场景下就需要为map加锁来保证并发的安全性了，Go语言的sync包中提供了一个开箱即用的并发安全版`map–sync.Map`。开箱即用表示**不用像内置的map一样使用make函数初始化就能直接使用**。同时`sync.Map`内置了诸如`Store、Load、LoadOrStore、Delete、Range`等操作方法。
```
var m = sync.Map{}
